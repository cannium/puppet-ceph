...
#
# Installs and configures first ceph monitor
### == Parameters
# [*title*] The MON id.
#   Mandatory. An alphanumeric string uniquely identifying the MON.
#
# [*fsid*] The Ceph cluster uuid
#
...
#
# [*mon_key*] Authentication key for monitor, in string format
#
# [*keyring*] Path of the keyring file of client.admin
#
define ceph::firstmon (
    $fsid,
    $ensure = present,
    $public_addr = undef,
    $cluster = undef,
...
) 
{
    ...
    if $ensure == present {
        ...
        File[$temp_keyring_path] -> Exec['import_admin_keyring'] -> Exec[$ceph_mkfs]
        file {$temp_keyring_path:
            mode        => '0600',
            content     => "[mon.]\n\tkey = ${mon_key}\n\tcaps mon = \"allow *\"\n",
        }
        exec {'import_admin_keyring':
            command => "/usr/bin/ceph-authtool ${temp_keyring_path} --import-keyring ${admin_keyring}",
            logoutput => true,
        }

        exec {'init_monitor_map':
            command => "/usr/bin/monmaptool --create --add ${::hostname} ${public_addr} --fsid ${fsid} /tmp/monmap --clobber",
